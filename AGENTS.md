...
- **`main.rs` is untestable:** `src/main.rs` is the binary crate — nothing in it can be reached from a test. Any `cmd_*` logic worth testing belongs in the library (`src/module/`, `src/core/`), with `main.rs` left holding argument plumbing and `println!`. See `manager::prune_modules` and `cmd_prune`.
- **Catalog `icon` values are invisible:** they are Nerd Font Private Use Area glyphs, which the Read tool shows as `icon = ""`. Rewriting a manifest from what you read silently empties the icon and fails `catalog_manifests_all_parse`. Edit around the line, or write it as a `U+XXXX` escape.
- **Catalog globs under shared vendor directories:** `JetBrains/`, `Google/` and the like hold unrelated siblings (Toolbox's installed apps, Chrome), so never end a pattern there with a bare `*`. Pin the name to a product prefix or version shape, and assert the siblings don't match in a catalog test, as `jetbrains_patterns_match_only_ide_version_folders` does.
- **Manual CLI/TUI verification uses a scratch `HOME`:** `dirs::home_dir()` honours `$HOME`, so `HOME=$(mktemp -d) ./target/debug/freespace ...` gives a throwaway config and modules directory. Never run `module prune`, `module remove`, or a cleanup against the real `$HOME`. For the TUI, drive it through a pty (`pty.fork()` plus `TIOCSWINSZ`) and send `q`.
